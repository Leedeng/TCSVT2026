"""GRPO grounding-control variant (rebuttal R1-2 / R2-4).

Same per-class GRPO as grpo_train.py, plus a --centroid_mode switch that corrupts
the visual signal used by the reward to test whether the gain is genuinely due to
visual grounding:
  real     : true per-class visual centroids/samples (== grpo_train.py).
  permuted : shuffle the class<->visual assignment (a class's description is
             rewarded against another class's real visual data).
  random   : replace all visual embeddings with random unit vectors.
If the reward truly encodes visual grounding, 'permuted'/'random' should collapse
the downstream accuracy back to the no-GRPO baseline.
"""
import argparse
import json
import random
...
def main():
    ...
    parser.add_argument("--centroid_mode", type=str, default="real",
                        choices=["real", "permuted", "random"],
                        help="Grounding control (R1-2/R2-4). 'real': true per-class visual "
                             "centroids/samples. 'permuted': shuffle the class<->visual "
                             "assignment. 'random': replace visual embeddings with random unit "
                             "vectors. Non-real modes should collapse the gain if the reward "
                             "truly encodes visual grounding.")
    ...
    # --- Grounding control (R1-2 / R2-4): optionally corrupt the visual signal ---
    # 'perm' maps each generated class to the class whose visual data is used as its
    # reward target. real -> identity; permuted -> a random derangement.
    perm = list(range(num_classes))
    if args.centroid_mode == "random":
        mean_image_embs = F.normalize(torch.randn_like(mean_image_embs), dim=-1)
        for c in class_embs:
            class_embs[c] = F.normalize(torch.randn_like(class_embs[c]), dim=-1)
        print("[centroid_mode=random] replaced all visual embeddings with random unit vectors")
    elif args.centroid_mode == "permuted":
        g = torch.Generator().manual_seed(args.seed)
        perm = torch.randperm(num_classes, generator=g).tolist()
        for i in range(num_classes):  # derange: eliminate fixed points
            if perm[i] == i:
                j = (i + 1) % num_classes
                perm[i], perm[j] = perm[j], perm[i]
        n_fixed = sum(1 for i in range(num_classes) if perm[i] == i)
        print(f"[centroid_mode=permuted] class->visual-target permutation applied "
              f"(remaining fixed points: {n_fixed})")
    ...
    for epoch in range(args.epochs):
        ...
        for c in pbar:
            # Reward target = perm[c]: identity for 'real', shuffled for 'permuted'.
            # The prompt still uses labels[c] (descriptions are about gesture c), but the
            # visual data they are optimized against belongs to class perm[c].
            tgt = perm[c]
            loss, mean_reward, reward_dict, best_desc = grpo_class_step(
                ...
                class_embs[tgt], labels[c], tgt, mean_image_embs,
